...
start_server() {
    ...
    if curl -sf http://localhost:${MCP_DEV_PORT:-$DEFAULT_PORT}/health > /dev/null 2>&1; then
        print_msg "$GREEN" "[OK] Dev MCP Server is running on port ${MCP_DEV_PORT:-$DEFAULT_PORT}"
    else
...
show_usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  start    - Start the Dev MCP server"
